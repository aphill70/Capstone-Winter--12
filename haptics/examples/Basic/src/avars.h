...
ENGINE_VAR debug_result;
ENGINE_VECTOR nullvector;
ENGINE_VAR protocol;
ENGINE_VAR reserved00[6];

// time & frame
ENGINE_VAR time;			// smoothed time of the last frame
...
ENGINE_VAR reserved07[6];

// renderer
ENGINE_VAR d3d_transform;
ENGINE_VAR d3d_mode;
...
ENGINE_VAR sky_clip;
ENGINE_VAR reserved22[4];
ENGINE_VAR sun_light;
ENGINE_COLOR sun_color;
ENGINE_VAR sun_angle;
ENGINE_VAR sun_pos;
...
ENGINE_MATERIAL	render_mtl;		// current entity material
...
long  (*GetMessage)(UINT message, WPARAM w_param, LPARAM l_param);	// user provided message handler
...
BOOL  isPointer;		// TRUE if GetObj returned a pointer rather than the object
void  *Functions;		// Null-terminated ENGINE_FUNCTION list 
long  eunused3[10];

// DirectX interfaces ////////////////////////////////////////////////
void* pdi;			// pointer to the DirectInput object 
void* pdimouse;		// pointer to the DirectInput mouse device
void* pdikbd;		// not used - keyboard doesn't use DirectInput
void* pdijoy;		// not used  -joystick doesn't use DirectInput

void* pdplayclient; // pointer to the DirectPlayClient interface 
void* pdplayserver; // pointer to the DirectPlayServer interface 
void* dunused1[2];	// not used 

void* pds;			// pointer to the IDirectSound interface
void* pdsb;			// pointer to the primary IDirectSoundBuffer

void* pd3d;			// pointer to the Direct3D interface 
void* pd3ddev;		// pointer to the IDirect3DDevice
HWND  hWnd;			// main app window
void* pd3dcaps;		// pointer to the current D3DCAPS
void* dunused2[9];	// not used

void* pdsbs;		// pointer to the recently created secondary IDirectSoundBuffer
